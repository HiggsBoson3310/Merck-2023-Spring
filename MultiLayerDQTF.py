...
class Qlearner():
    def __init__(self, dim_i, dim_l, dim_o, alpha=1e-4,gamma=0.2,decay_rate=0.98,decay_steps=1000):
        self.alpha=alpha
    ...
    def create_network(self):
        self.M0 = self.weight_variable(self.dim_i,self.dim_l[0])
        self.b0 = self.bias_variable(1,self.dim_l[0])#self.weight_variable(1,self.dim_l)
        self.Mh = self.weight_variable(self.dim_l[0],self.dim_l[1])
        self.bh = self.bias_variable(1,self.dim_l[1])
        self.M1 = self.weight_variable(self.dim_l[1],self.dim_o)
        self.b1 = self.bias_variable(1,self.dim_o)#self.weight_variable(1,self.dim_o)
        self.trainable_vars = [self.M0,self.Mh,self.M1,self.b0,self.bh,self.b1]
    ...
    @tf.function
    def qvalues(self,M0,b0,Mh,bh,M1,b1,state_input):
        L0 = tf.nn.relu(tf.matmul(state_input,M0)+b0)
        L1 = tf.nn.relu(tf.matmul(L0,Mh)+bh)
        return tf.matmul(L1,M1)+b1
    ...
    @tf.function
    def loss(self,M0,b0,Mh,bh,M1,b1,y_input,action_inputs,states_input):
        qvals = self.qvalues(M0,b0,Mh,bh,M1,b1,states_input)
    ...
    def Training_network(self,y_batch_input,action_batch,state_batch):
        with tf.GradientTape() as tp:
            loss_int = self.loss(self.M0,self.b0,self.Mh,self.bh,self.M1,self.b1,y_batch_input,action_batch,state_batch)
        gradients = tp.gradient(loss_int,self.trainable_vars)
        self.optimizer.apply_gradients(zip(gradients,self.trainable_vars))
        self.current_loss = self.loss(self.M0,self.b0,self.Mh,self.bh,self.M1,self.b1,y_batch_input,action_batch,state_batch)
